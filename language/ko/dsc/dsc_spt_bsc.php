<div class="box">
<a name="internet_setup"><h2>인터넷 설정</h2></a>
...
<tr>
	<td height=59>
		<p>
			...
			아래 버튼을 클릭하면 몇가지 간단한 단계를 거쳐 인터넷에 연결 됩니다.
			<br>
			<strong>무선 보안 설정 마법사</strong><br>
			아래 버튼을 클릭하면 무선 네트워크 설정 및 보안 설정하는 방법을 안내할 것입니다. 
		</p>
	</td>
</tr>
...
<tr>
	<td height=46>
		<p>
			...
			직접 인터넷 연결 설정을 하시려면 이 옵션을 선택하세요.
			<br>
			<strong>엑세스 포인트 모드</strong><br>
			엑세스 포인트 모드를 선택하면 이 장비는 무선 AP 처럼 동작합니다.  무선 연결을 제외한 모든 NAT 기능은 동작하지 않습니다. 
			<br>
			<strong>인터넷 연결 타입</strong><br>
			공유기를 이용하여 인터넷에 연결하려면 메뉴에서 정확한 내 인터넷 연결 을 설정해야 합니다. 어떤 옵션을 설정해야 할지 확실하지 않을 경우에는 인터넷 서비스 업체(ISP) 에 문의하시기 바랍니다.
		</p>
	</td>
</tr>
...
<tr>
	<td height=61>
		<p>
			...
			이 옵션을 선택하면 ISP업체에서 제공한 IP 어드레스, 서브넷 마스크, 디폴트 게이트웨이 그리고 DNS 서버 어드레스 정보를 입력해야 하고,이런 정보가  확실하지 않을 경우에는 인터넷 서비스 업체(ISP) 에 문의하시기 바랍니다.
		</p>
	</td>
</tr>
<tr><td height=20>&nbsp;</td></tr>
<tr>
	<td height=142>
		<p>
			...
			이 옵션을 선택하면 ISP업체에서 자동으로 IP 어드레스가 부여 됩니다. 케이블 모뎀 공급사에서도 동적 IP 어드레스가 할당됩니다.
		</p>
		<p>
			<strong><em>호스트 네임</em></strong> (옵션) -
			호스트 네임은 옵션이고 ISP 업체에 의해 필요하기도 합니다. 디폴트 호스트 네임은 장비의 모델 넘버입니다.
			<br>
			<strong><em>맥 어드레스 </em></strong> (옵션) -
			맥(미디어 액세스 컨트럴) 어드레스 필드는 ISP 업체에서 필요로 합니다. 디폴트 맥 어드레스는 <?query("/sys/modelname");?> WAN 인터페이스 맥어드레스로 설정됩니다. "PC MAC 주소 복사"버튼을  사용하면 컴퓨터의 이더넷 카드 맥 어드레스가 자동으로 복사가 됩니다. ISP 업체에서 요구 할때만 필드에 주소를 복사합니다 
			<br>
			<strong><em>프라이머리 DNS 어드레스 </em></strong> -
			프리이머리 DNS(도메인 네임 서비스)서버 IP 어드레스를 입력합니다.<br>
...
<tr>
	<td height=47>
		...
		<p>
			...
			이 옵션은 스태틱 PPPoE를 사용할 경우만 유효합니다. 스태틱 PPPoE 연결하려면 정적 IP 어드레스를 입력하세요<br>
			<strong><em>맥 어드레스 </em></strong> (옵션) -
			맥(미디어 액세스 컨트럴) 어드레스 필드는 ISP 업체에서 필요로 합니다. 디폴트 맥 어드레스는 <?query("/sys/modelname");?> WAN 인터페이스 맥어드레스로 설정됩니다. "PC MAC 주소 복사"버튼을  사용하면 컴퓨터의 이더넷 카드 맥 어드레스가 자동으로 복사가 됩니다. ISP 업체에서 요구 할때만 필드에 주소를 복사합니다 
			<br>
			<strong><em>프라이머리 DNS 어드레스 </em></strong> -
			ISP에서 제공한 프라이머리 DNS(도메인 네임 서비스)서버 IP 어드레스. 스태틱 PPPoE를 선택할때 이 정보가 필요합니다. 만일 다이나믹 PPPoE를 사용한다면 자동으로 ISP 업체에서 이 정보를 제공합니다.
			<br>
			<strong><em>세컨더리  DNS 어드레스</em></strong> (옵션) -
			세컨더리 DNS 서버 IP가 있다면 입력하세요<br>
			<strong><em>최대 유휴시간</em></strong> -
			디바이스가 PPPoE  세션이 끊기기 전에 비활동 시간. 최대 유휴시간을 정의하면 비활동중에서도 인터넷 연결이 유지된다. 비활동시간이 최대유휴시간을 초과하면 연결이 끊김니다. 이 옵션은 연결 요구 모드에서만 적용됩니다.
			<br>
			<strong><em>MTU </em></strong> -
			MTU(최대 전송/전달 단위)는 네트워크를 통해 보낼수 있는 최대 크기의 패킷 사이즈입니다. MTU 보다 큰 메시지는 작은 패킷으로 나누어 집니다. 1492 이 디폴트 값입니다. 이 값을 변경하면 라우터의 성능에 영향을 미칠수 있습니다. 기술지원부서나 ISP 업체에 요구에 의해서만 변경할수 있습니다.
			<br>
...
<tr>
	<td height=20>
		...
		<p>
			...
			<strong><em>최대 유휴시간 </em></strong> -
			디바이스가 PPTP  세션이 끊기기 전에 비활동 시간. 최대 유휴시간을 정의하면 비활동중에서도 인터넷 연결이 유지된다. 비활동시간이 최대유휴시간을 초과하면 연결이 끊김니다. 이 옵션은 연결 요구 모드에서만 적용됩니다.
			<br>
			<strong><em>MTU </em></strong> -
...
<tr>
	<td height=20>
		...
		<p>
			...
			<strong><em>L2TP패스워드</em></strong> - L2TP 패스워드 입력<br>
			<strong><em>최대 유휴시간 </em></strong> -
			디바이스가 L2TP  세션이 끊기기 전에 비활동 시간. 최대 유휴시간을 정의하면 비활동중에서도 인터넷 연결이 유지된다. 비활동시간이 최대유휴시간을 초과하면 연결이 끊김니다. 이 옵션은 연결 요구 모드에서만 적용됩니다.
			<br>
			<strong><em>MTU</em></strong> -
			MTU(최대 전송/전달 단위)는 네트워크를 통해 보낼수 있는 최대 크기의 패킷 사이즈입니다. MTU 보다 큰 메시지는 작은 패킷으로 나누어 집니다. 1400 이 디폴트 값입니다. 이 값을 변경하면 라우터의 성능에 영향을 미칠수 있습니다. 기술지원부서나 ISP 업체에 요구에 의해서만 변경할수 있습니다.
			<strong><em>연결 모드 선택</em></strong> -
...
<div class="box">
<a name="wireless_setup"><h2>무선 설정</h2></a>
<table border=0 cellspacing=0 cellpadding=0>
<tr>
	<td height=20>
		<p>
...
<tr>
	<td height=20>	
 		<p>
 			<strong>전송(TX)률</strong><br>
 			WLAN(무선 로컬 에어리어 네트워크)의 무선 어댑터의 전송률을 선택하세요
 		</p>
	</td>
</tr>
<tr><td height=20>&nbsp;</td></tr>
...
<tr>
	<td height=20>
		<p>
			...
			<?query("/sys/modelname");?>의 SSID가 브로드캐스트 하고 싶지 않으면 선택하세요. 이 옵션을 동작시키면, <?query("/sys/modelname");?>의 SSID는 Site Survey 유틸리티에 보이지 않습니다. 무선 클라이언트가 접속을 할려면 <?query("/sys/modelname");?>의 SSID를 알아라 하고, 수동으로 입력을 해야 합니다. 디폴트로는 동작상태입니다.
		</p>
	</td>
</tr>
<tr><td height=20>&nbsp;</td></tr>
...
<tr>
	<td height=20>
		<p>
			...
			Wired Equivalent Protocol (WEP) 는 무선 로컬 에어리어 네트워크(WLAN)에 사용되는 보안 프로토콜 입니다. WEP는 정보를 WLAN을 통해 인크립션을 해서 보냅니다. <?query("/sys/modelname");?>는 64-bit, 128-bit 두 가지 종류의 WEP 인크립션을 지원합니다.  디폴트는 비동작입니다. WEP는 무선 네트워크 커스터 마이징 하기 위해 변경 할수 있습니다.
		</p>
		<p>
...
<tr>
	<td height=20>
		<p>
			...
			Wi-Fi 프로텍트 엑세스(또는 Wi-Fi 프로텍트 엑세스 2)는 무선 네트워크 사용자에게 인증과 권한을 부여합니다. WPA는 WEP 보다 더 강한 보안을 지원하고, 키 값을 기반으로 하며 일정한 간격을 두고 자동으로 변경됩니다.
		</p>
		<p>
			<strong><em>암호화 타입</em></strong> -
			WPA를 사용할때 <?query("/sys/modelname");?>은 두가지 암호화 타입이 지원됩니다.  두가지 옵셥은 TKIP (Temporal Key Integrity Protocol) 과 AES (Advanced Encryption Standard) 입니다.
			<br>
			<strong><em>PSK/EAP</em></strong> -
			PSK를 선택하면 인증을 위해서 무선 클라이언트에 Pass phrase가 제공되어야 합이다. EAP를 선택하면, 인증을 위한 모든 무선 클라이언트를 위해서 RADIUS 서버가 필요합니다.
			<br>
			<strong><em>Passphrase</em></strong> -
			PSK가 선택되고 8-63 alphanumeric characters가 입력했을때 무선 클라이언트가 <?query("/sys/modelname");?>와 통신하기위한 것입니다. 다른 무선 장치를 네트워크에 추가 시키려면 Passphrase를 반드시 입력을 해야 합니다. 
			<br>
			<strong><em>802.1X</em></strong> -
			RADIUS 서버를 사용할때 WPA 인증이 결합해서 사용됩니다. RADIUS에는 IP 어드레스, 포트, 공유보안이 입력됩니다. 세컨더리 RADIUS 서버도 사용할 수 있습니다.
		</p>
	</td>
</tr>
</table>
...
<div class="box">
<a name="lan_setup"><h2>네트워크 설정</h2></a>
<table border=0 cellspacing=0 cellpadding=0>
<tr>
	<td height=20>
		<p>
			LAN (Local Area Network) 인터페이스를 설정을 합니다. 프라이비트 설정이라고도 합니다. 필요하다면 LAN IP 어드레스를 바꿀수 있습니다. 사설 IP는 인터넷에서 볼수는 없습니다.디폴트 IP 어드레스는 192.168.0.1이고 서브넷 마스크는 255.255.255.0 입니다.
			<br><br>
			<strong><em>IP 어드레스</em></strong> -
...
<tr>
	<td height=20>
		...
		<p>
			...
			수동으로 컴퓨터 이름, IP 어드레스, 맥 어드레스를 입력할 수 있고, 메뉴에서 현재 DHCP 클라이언트를 볼수 있습니다. 
		</p>
		<p>
		...
		<p>
			다이나믹 DHCP 클라이언트 테이블에 다이나믹 DHCP 클라이언트 컴퓨터 정보가 표시 됩니다. 테이블에는 호스트 네임, IP 어드레스, å 어드레스, DHCP 리즈 타임을 보여 줍니다.
		</p>
	</td>
</tr>
</table>
...
<tr>
	<td height=20>
		<p>
			LCD 설정 페이지는 <?query("/sys/modelname");?> 디스플레이 패널이 표시됩니다. 스크린세이버 또는 LCD 스크린의 유휴시간을 조절할 수 있습니다.
			<br><br>
			<strong><em>Screen Saver 설정</em></strong> -
			하드 디스크 루트 폴더에 저장된 "screen_bg.jpg"을 날짜와 시간과 같이 보여 줍니다. 그림을 찾지 못하면 블랙으로 표시됩니다. "Timeout" 설정은 스크린세이버가 동작하기 전까지의 요구 시간을 나타내고, 유휴시간이 남아 있는 상태에서 터치패드를 누르면 스크린세이버에서 빠져 나옵니다.
			<br>
			<strong><em>디스플레이 설정</em></strong> -
			파워를 세이브하기 위해 설정된  "Timeout"시간이 지나면 LCD 디스플레이가 off 가 됩니다. 터치패드를 누르면 디스플레이가 on 됩니다.
			<br>
		</p>
	</td>
</tr>
