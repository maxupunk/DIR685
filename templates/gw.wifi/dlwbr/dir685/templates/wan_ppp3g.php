...
/*	if (query("mode")==1)	{ $ppp_staticip = query("staticip"); }
	...
	if ($ppp_reconnmode == 2){ $ppp_persist=1; $ppp_demand=1; }
	...
	/* mark by bison */
	/* unmark by mark */
	if ($ppp_persist == 1 && $dev_link == "1" )
	{
		echo "sh /var/run/ppp-".$ppp_linkname.".sh start > /dev/console\n"; 
	}
	else if ($ppp_persist == 1 && $ppp_modem != "" && $ppp_modem != "0") 
	{ 
		echo "sh /var/run/ppp-".$ppp_linkname.".sh start > /dev/console\n"; 
	}
...
//	echo "echo \"keep_alive_sleep_interval=".$keep_alive_sleep_interval."\"\n");
	...
	if ($ppp_persist == 1)
	{ 
		echo "if [-f /var/run/keep_alive.sh ]; then \n";
