...
//	echo "f_xr=".				$f_xr.				"\n";
	...
	echo "f_band_width=". 		$f_band_width. 		"\n";
	echo "f_shortguard=". 		$f_shortguard. 		"\n";
	echo "-->\n";
	...
	if($f_enable=="1")
	{
		if(query("schedule/id")	!= $f_wlan_sch)			{set("schedule/id", $f_wlan_sch); 		$sch_dirty++;}
		...
		if(query("wlanmode")	!= $f_wlanmode)			{set("wlanmode",	$f_wlanmode);		$db_dirty++;}
		if($support11n!="1")
		{
		...
		{
			$f_wlanmode = query("wlanmode");
			if($f_wlanmode=="4" || $f_wlanmode=="6" || $f_wlanmode=="7" || $f_wlanmode=="12") //n mode enabled
			{
				if(query("bandwidth") != $f_band_width) 
				{set("bandwidth", 			$f_band_width);         $db_dirty++;}
				if(query("shortguardinterval") != $f_shortguard) 
				{set("shortguardinterval", 	$f_shortguard);         $db_dirty++;}
				if(query("wmm") != "1")
				{set("wmm", 				"1");					$db_dirty++;} // force wmm to be on in n mode
			}
			
			if($f_wlanmode=="1" || $f_wlanmode=="2" || $f_wlanmode=="8") //b, g, or a only mode
			{
				if(query("txrate") != $f_txrate)
				{
					set("txrate", $f_txrate);
			...
			else if($f_wlanmode=="4") //n only  mode
			{
				if($f_txrate=="0")
				{
					if(query("mcs/auto")!="1")
			...
			else // bg mix, bgn mix, or an mix mode
			{
				if(query("txrate")!="0" || query("mcs/auto")!="1" || query("mcs/index")!="")
				{
					set("txrate", "0");
					set("mcs/auto", "1");
					set("mcs/index", "");
...
//		}
	...
	else if($sch_dirty > 0) 	{$SUBMIT_STR="submit WLAN_SCH";}
	$NEXT_PAGE=$MY_NAME;
...
//		f.wlanband[0].disabled = false;
//		f.wlanband[1].disabled = false;
		<?
		if($support11n=="1")
		{
			echo "f.wlan_mode.disabled = false;\n";
			echo "f.wlan_mode_11a.disabled = false;\n";
		}
		else
		{
			echo "f.gonly.disabled = false;\n";
			echo "f.aonly.disabled = false;\n";
		}
		?>
		if (f.autochann.checked) 
		{
			f.channel.disabled = true;
			f.channel_11a.disabled = true;
		}
		else 
		{
			f.channel.disabled = false;
			f.channel_11a.disabled = false;
		}
		...
		{
//			echo "wlanModeChg();\n";
			if($wlanMode=="4" || $wlanMode=="6" || $wlanMode=="7") //when 11n,always enable WMM
			{
...
function chg_wlan_band()
{
	var f = get_obj("frm");
//	if(f.wlanband[1].checked)
//	{
//		get_obj("show_2g_mode").style.display = "none";
//		get_obj("show_2g_channel").style.display = "none";
//		get_obj("show_5g_mode").style.display = "";
//		get_obj("show_5g_channel").style.display = "";
		<?
//		if($support11n=="1")
//		{
//			if( $cfg_wlanmode=="" || $cfg_wlanmode!="8" || $cfg_wlanmode!="12" )
//			{
//				echo "select_index(f.wlan_mode_11a, \"12\");\n";
//			}
//			else
//			{
//				echo "select_index(f.wlan_mode_11a, \"".$cfg_wlanmode."\");\n";
//			}
//		}
		?>
//	}
//	else
//	{
		get_obj("show_2g_mode").style.display = "";
		get_obj("show_2g_channel").style.display = "";
		get_obj("show_5g_mode").style.display = "none";
		get_obj("show_5g_channel").style.display = "none";
		<?
		if($support11n=="1")
		{
			if( $cfg_wlanmode=="" || $cfg_wlanmode=="8" || $cfg_wlanmode=="12" )
			{
				...
				{ echo "select_index(f.wlan_mode, \"6\");\n"; }
				else
				{ echo "select_index(f.wlan_mode, \"7\");\n"; }
			}
			else
			{
				echo "select_index(f.wlan_mode, \"".$cfg_wlanmode."\");\n";
			}
		}
		?>
//	}
...
function on_check_autochann()
{
	...
	f.channel_11a.disabled = f.autochann.checked;
}
...
function init()
{
	...
	<? 
	//check the corresponding radio button depending on if it is 11a band or not
	if($cfg_wlanmode=="8" || $cfg_wlanmode=="12")
	{
//		echo "f.wlanband[1].checked=true;\n";
		if($support11n=="1")
		{
			echo "select_index(f.wlan_mode_11a, \"".$cfg_wlanmode."\");\n";
		}
		echo "select_index(f.channel_11a, \"".$cfg_channel."\");\n";
	}
	else
	{
//		echo "f.wlanband[0].checked=true;\n";
		if($support11n=="1")
		{
			echo "select_index(f.wlan_mode, \"".$cfg_wlanmode."\");\n";
		}
		echo "select_index(f.channel, \"".$cfg_channel."\");\n";
	}
	?>
...
//	}
	...
	{
		$wlanmode = query("/wireless/wlanmode");
		if($wlanmode=="4" || $wlanmode=="6" || $wlanmode=="7" || $wlanmode=="12") //when 11n,always enable WMM
		{
			echo "f.wmm_enable.disabled = true;\n";
		}
	}
...
function strchk_ralink_ssid(str)
{
	var char_code;
	var i;
...
//	f_final.f_xr.value				="";
	...
	f_final.f_wlanmode.value 		="";
	f_final.f_band_width.value 		="";
	f_final.f_shortguard.value 		="";
	...
	if(strchk_ralink_ssid(f.ssid.value))
	{
		alert("<?=$a_invalid_ssid?>");
		f.ssid.focus();
		return false;
...
//	if(!f.wlanband[0].checked && !f.wlanband[1].checked)
//	{
//		alert("<?=$a_invalid_wlan_band?>");
//		f.wlanband.focus();
//		return false;
//	}
//	if(f.wlanband[0].checked) /* b/g band */
//	{
		<?
		if($support11n!="1")
		{
			echo "f_final.f_wlanmode.value =(f.gonly.checked? \"2\":\"3\");\n";
		}
		else
		{
			echo "f_final.f_wlanmode.value = f.wlan_mode.value;\n";
		}
		?>
...
//		if($support11n!="1")
//		{
//			echo "f_final.f_wlanmode.value = f.aonly.value;\n";
//			echo "alert(\"f_final.f_wlanmode.value =\"+f_final.f_wlanmode.value);\n";
//		}
//		else
//		{
//			echo "f_final.f_wlanmode.value = f.wlan_mode_11a.value;\n";
//		}
		?>
//		f_final.f_channel.value			=f.channel_11a.value;
//	}
	...
	if($support11n=="1")
	{
		echo "f_final.f_band_width.value = f.band_width.value;\n";
		echo "f_final.f_shortguard.value = (f.short_guard.checked? \"1\":\"0\");\n";
	}
	?>
...
//	var wlanMode = "<?=$wlanMode?>";
	var wlanMode = get_obj("frm").wlan_mode.value;
	...
	new_txrate_txt = "<select name=\""+id+"\" id=\""+id+"\">\n";
	new_txrate_txt = new_txrate_txt+"<option value=\"0\"><?=$m_best_auto?></option>\n";
	for( inx = 0; inx < listOptions.length; inx ++ )
	{
		if( type == "11g" )
			new_txrate_txt = new_txrate_txt+"<option value=\""+listOptions[inx]+"\">"+listOptions[inx]+"</option>\n";
		else
			new_txrate_txt = new_txrate_txt+"<option value=\""+(inx+1)+"\">"+listOptions[inx]+"</option>\n";
	}
	new_txrate_txt = new_txrate_txt+"</select>\n";
	get_obj("txrate_txt").innerHTML = new_txrate_txt;
}
<?
if($support11n=="1")
{
...
echo "	if(f.wlan_mode.value!=\"4\" && f.wlan_mode.value!=\"7\" && f.wlan_mode.value!=\"6\" && f.wlan_mode_11a.value!=\"12\")\n";
echo "	{\n";
...
echo " 		f.wmm_enable.checked = true;\n";
echo "		f.wmm_enable.disabled = true;\n";
echo "	}\n";
...
<input type="hidden" name="f_wlanmode" 			value="">
<input type="hidden" name="f_band_width" 		value="">
<input type="hidden" name="f_shortguard" 		value="">
</form>
...
<?
						...
						$OBJID		= "wlan_sch";
						$OBJNAME	= "wlan_sch";
						$UNIQUEID	= query("/wireless/schedule/id");
...
<!--
			<tr>
				<?=$td1?><?=$m_wlan_band?></td>
				<?=$td2?><?=$symbol?>
					<input type="radio" value="0" id="wlanband" name="wlanband" onclick="chg_wlan_band()"><?=$m_wlan_band_2g?>
					<input type="radio" value="1" id="wlanband" name="wlanband" onclick="chg_wlan_band()"><?=$m_wlan_band_5g?>
				</td>
			</tr>
-->
			<tr>
                <?=$td1?><?=$m_wlan_mode_set?></td>
                <?=$td2?><?=$symbol2?>
				<span id="show_2g_mode" style="display:none">
					<?
						if($support11n=="1") { echo "<!--\n"; }
					?>
					<?=$m_11g_only?><input name='gonly' id='gonly' type='checkbox' value='2'>
					<?
						if($support11n=="1") { echo "-->\n"; }
					?>
					<?
						...
						{	
							...
							if(query("/runtime/func/dis_wlan_ng")!="1")
							{	echo "<option value=\"6\">".$m_wlan_mode_mixgn."</option>\n";	}
							if(query("/runtime/func/dis_wlan_nbg")!="1")
				...
				<span id="show_5g_mode" style="display:none">
					<?
						if($support11n=="1") { echo "<!--\n"; }
					?>
					<?=$m_11a_only?>
					<input type="hidden" name='aonly' id='aonly' value='8'>
					<?
						if($support11n=="1") { echo "-->\n"; }
					?>
					<?
						if($support11n=="1")
						{	
							echo "<select name=\"wlan_mode_11a\" onChange=\"wlanModeChg();\">\n";
							if(query("/runtime/func/dis_wlan_a")!="1")
							{	echo "<option value=\"8\">".$m_wlan_mode_11a."</option>\n";}
							if(query("/runtime/func/dis_wlan_an")!="1")
							{	echo "<option value=\"12\">".$m_wlan_mode_11an."</option>\n";}
							echo "</select>\n";
						}
					?>
				</span>
			...
			<tr>
				...
				<?=$td2?><?=$symbol2?>
				<span id="show_2g_channel" style="display:none">
					<select name="channel" id="channel">
						<?require("/www/bsc_wlan_channel.php");?>
					</select>
				</span>
				<span id="show_5g_channel" style="display:none">
					<select name="channel_11a" id="channel_11a">
						<?require("/www/bsc_wlan_channel_11a.php");?>
					</select>
				</span>
				</td>
			</tr>
			...
			<tr>
				...
				-->
				<span id="txrate_txt" name="txrate_txt">
					<select name="txrate" id="txrate">
						<option value="0"><?=$m_best_auto?></option>
					</select>
				</span>
				<?=$m_mbps?>
			...
			if($support11n=="1")
			{
				...
				echo "</tr>\n";
				echo "<tr>\n";
				echo $td1.$m_wlan_short_guard."</td>\n";
				echo $td2.$symbol."\n";
