...
$support3g =query("/runtime/func/wwan_3g");
...
if ($ACTION_POST!="")
{
  ...
  echo "<!--\n";
	...
	else if ($ACTION_POST == "WWAN")
	{
		/* echo "carddevice=".$card_device."\n"; */
		echo "pincode=".$pin_code."\n";
		echo "username=".$username."\n";
		echo "password=".$password."\n";
		echo "dialnumber=".$dial_num."\n";
		echo "authproto=".$auth_proto."\n";
		echo "apn=".$apn."\n";
		echo "mtu=".$mtu."\n";
		echo "idletime=".$idletime."\n";
		echo "reconnmode=".$reconnmode."\n";
		/*
		echo "autoreconnect=".$auto_reconn."\n";
		echo "keepaliveint=".$keep_alive_int."\n";
		echo "keepaliveserver1=".$keep_alive_server1."\n";
		echo "keepaliveserver2=".$keep_alive_server2."\n";
		*/
		/* echo "idletimeout=\n"; */

		$entry="/wan/rg/inf:1/ppp3g/";
		if (query("/wan/rg/inf:1/mode")	!="6") 						{$dirty++; set("/wan/rg/inf:1/mode", "6");}
		/* if (query($entry."carddevice") !=$card_device) 				{$dirty++; set($entry."carddevice", $card_device);} */
		if (query($entry."country") !=$country)						{$dirty++; set($entry."country", $country);}
		if (query($entry."ips") !=$isp)								{$dirty++; set($entry."isp", $isp);}
		if (query($entry."pincode") !=$pin_code)					{$dirty++; set($entry."pincode", $pin_code);}
		if (query($entry."username") !=$username) 					{$dirty++; set($entry."username", $username);}
		if (query($entry."password") !=$password) 					{$dirty++; set($entry."password", $password);}
		if (query($entry."dialnumber")	!=$dial_num)				{$dirty++; set($entry."dialnumber", $dial_num);}
		if (query($entry."authproto") !=$auth_proto) 				{$dirty++; set($entry."authproto", $auth_proto);}
		if (query($entry."apn") !=$apn) 							{$dirty++; set($entry."apn", $apn);}
		if (query($entry."mtu") !=$mtu) 							{$dirty++; set($entry."mtu", $mtu);}
		/*
		if (query($entry."autoreconnect") !=$auto_reconn) 			{$dirty++; set($entry."autoreconnect", $auto_reconn);}
		if ($auto_reconn == 0) 										{set($entry."ondemand", 0);}
		if (query($entry."keepaliveint") !=$keep_alive_int) 		{$dirty++; set($entry."keepaliveint", $keep_alive_int);}
		if (query($entry."keepaliveserver1") !=$keep_alive_server1) {$dirty++; set($entry."keepaliveserver1", $keep_alive_server1);}
		if (query($entry."keepaliveserver2") !=$keep_alive_server2) {$dirty++; set($entry."keepaliveserver2", $keep_alive_server2);}
		if (query($entry."idletimeout") == "") 						{$dirty++; set($entry."idletimeout", 120);}
		*/
		if (query($entry."reconnmode") !=$reconnmode)               {$dirty++; set($entry."reconnmode", $reconnmode);}
		if ($reconnmode=="2")
		{
			$idletime = $idletime * 60;
			if (query($entry."idletimeout") !=$idletime)                {$dirty++; set($entry."idletimeout", $idletime);}
			if (query($entry."onDemand") != 1) 						{$dirty++; set($entry."onDemand"), 1);}
			if (query($entry."autoReconnect") != 1) 					{$dirty++; set($entry."autoReconnect"), 1);}
		}
		else if ($reconnmode=="1")
		{
			if (query($entry."onDemand") != 0) 						{$dirty++; set($entry."onDemand"), 0);}
			if (query($entry."autoReconnect") != 0) 					{$dirty++; set($entry."autoReconnect"), 0);}
		}
		else
		{
			if (query($entry."onDemand") != 0) 						{$dirty++; set($entry."onDemand"), 0);}
...
if($cfg_poe_user=="")	{$cfg_poe_pass = "";}
else					{$cfg_poe_pass = $G_DEF_PASSWORD;}
...
if($cfg_pptp_user=="")	{$cfg_pptp_pass = "";}
else					{$cfg_pptp_pass = $G_DEF_PASSWORD;}
...
if($cfg_l2tp_user=="")	{$cfg_l2tp_pass = "";}
else					{$cfg_l2tp_pass = $G_DEF_PASSWORD;}
...
$cfg_wwan_country			= query("country");
if($cfg_wwan_country=="")	{$cfg_wwan_country=-1;}
$cfg_wwan_isp				= query("isp");
if($cfg_wwan_isp=="")		{$cfg_wwan_isp=-1;}
$cfg_wwan_card_dev			= query("carddevice");
$cfg_wwan_pin_code			= query("pincode");
$cfg_wwan_username			= query("username");
$cfg_wwan_password			= query("password");
$cfg_wwan_dial_num			= query("dialnumber");
$cfg_wwan_auth_proto		= query("authproto");
$cfg_wwan_apn				= query("apn");
$cfg_wwan_mtu				= query("mtu");
$cfg_wwan_reconnmode		= query("reconnmode");
$cfg_wwan_idle_time			= query("idletimeout");
/*
$cfg_wwan_auto_conn			= query("autoreconnect");
$cfg_wwan_keep_alive_int	= query("keepaliveint");
$cfg_wwan_keep_alive_serv1	= query("keepaliveserver1");
$cfg_wwan_keep_alive_serv2	= query("keepaliveserver2");
*/
$cfg_wwan_idle_time            = $cfg_wwan_idle_time/60;
/* --------------------------------------------------------------------------- */
?>
<script>
...
function on_change_wan_type()
{
	...
	get_obj("show_wwan").style.display		= "none";
...
<?if($support3g=="1")
{
	echo "	case \"6\":		get_obj(\"show_wwan\").style.display		= \"\"; break;\n";
}?>
...
function check_dhcp()
{
	...
	if (!is_digit(f.mtu.value) || !is_in_range(f.mtu.value, 576, 1500))
	{
		alert("<?=$a_invalid_mtu?>");
		field_focus(f.mtu, "**");
		return false;
...
function clickPPP3G()
{
	...
	var modemtype = '<? map("/runtime/stats/usb/devices/driver","","0"); ?>';
	var modemmanu = '<? map("/runtime/stats/usb/devices/manufacturer","","Unknown"); ?>';
	var modemprod = '<? map("/runtime/stats/usb/devices/product","","Unknown"); ?>';
	switch (modemtype)
	{
		case "acm":
			break;
	...
	var div_temp = get_obj("div_modemtype");
	div_temp.innerHTML=modemtype; 
	var div_temp2 = get_obj("div_modemmanu");
	div_temp2.innerHTML=modemmanu; 
	var div_temp3 = get_obj("div_modemprod");
	div_temp3.innerHTML=modemprod; 

}
*/

function keep_alive_mode(form)
{
	var f = get_obj(form);
	if( f.auto_reconn[1].checked == true )
	{
		get_obj("keepalive_wwan").style.display = "none";
	}
	else
	{
		get_obj("keepalive_wwan").style.display = "";
	}
}

<?require("/www/__bsc_wan_3gisp.php");?>
...
/*
	f.auto_reconn.value = "<?=$cfg_wwan_auto_conn?>"
	if(f.auto_reconn.value != 0)
	{
		f.auto_reconn[0].checked = true;
	}
	else
	{
		f.auto_reconn[1].checked = true;
	}
...
<?
	if ($cfg_wwan_reconnmode == "0")        {echo "f.ppp_conn_mode[0].checked=true;\n";}
	else if ($cfg_wwan_reconnmode == "1")   {echo "f.ppp_conn_mode[1].checked=true;\n";}
	else                                    {echo "f.ppp_conn_mode[2].checked=true;\n";}
?>
	chg_wwan_conn_mode(f);

	// init 3gisp stuff (profiles)
	init_country();
...
/*
	if(f.card_device.selectedIndex == "0")
	{
		alert("<?=$a_card_device_not_selected?>");
		field_focus(f.card_device.value, "**");
		return false;
	} 
*/
...
*/
	...
	if(is_blank(f.dial_num.value))
	{
		alert("<?=$a_dial_num_empty?>"); 
		field_focus(f.dial_num, "**");
		return false;
	}
/*
	if(f.auth_proto.selectedIndex == "0")
	{
		alert("<?=$a_auth_proto_not_selected?>");
		field_focus(f.auth_proto.value, "**");
		return false;
	}
	if(is_blank(f.apn.value))
	{
		alert("<?=$a_apn_empty?>");
		field_focus(f.apn.value, "**");
		return false;
	}
	if(f.auto_reconn[0].checked == true && is_blank(f.keep_alive_int.value))
	{
		alert("<?=$a_keep_alive_int_empty?>");
		field_focus(f.keep_alive_int.value, "**");
		return false;
	}
	if(!is_blank(f.keep_alive_int.value) && (!is_digit(f.keep_alive_int.value) || !is_in_range(f.keep_alive_int.value, 20, 3600)) )
	{
		alert("<?=$a_invalid_keep_alive_int?>"); //TODO: bug "abc" passed the test
		field_focus(f.keep_alive_int.value, "**");
		return false;
	}
*/
	if (!is_digit(f.mtu.value) || !is_in_range(f.mtu.value, 128, 1492))
	{
		alert("<?=$a_invalid_mtu?>");
		field_focus(f.mtu, "**");
		return false;
	...
	if (f.ppp_conn_mode[0].checked)
	{
       f.reconnmode.value = "0";
	}
	else if (f.ppp_conn_mode[2].checked)
	{
	   if (!is_digit(f.idletime.value))
	    {
	        alert("<?=$a_invalid_idletime?>");
	    ...
	    f.reconnmode.value = "2";
	}
	else
	{
	    f.reconnmode.value = "1";
	}
...
<!--*********************************************-->
	...
	<!-- PPPoE  -->
		...
		<tr>
			...
			<td class="l_tb">&nbsp;
				<input type=password id=password name=password size=30 maxlength=255 value="<?=$cfg_poe_pass?>">
			</td>
        </tr>
		<tr>
			<td class="r_tb" width=150><?=$m_retype_pwd?> :</td>
			<td class="l_tb">&nbsp;
				<input type=password id=password_v name=password_v size=30 maxlength=255 value="<?=$cfg_poe_pass?>">
			</td>
		</tr>
		<tr>
...
<!------------------------------------->
	...
	<!-- PPTP  -->
		...
		<tr>
			...
			<td class="l_tb">&nbsp;
				<input type=password id=password name=password size=32 maxlength=255 value="<?=$cfg_pptp_pass?>">
			</td>
		</tr>
		<tr>
			<td class="r_tb" width=150><?=$m_pptp_retype_pwd?> :</td>
			<td class="l_tb">&nbsp;
				<input type=password id=password_v name=password_v size=32 maxlength=255 value="<?=$cfg_pptp_pass?>">
			</td>
		</tr>
		<tr>
			<td class="r_tb" width=150><?=$m_ppp_idle_time?> :</td>
...
<?						$PREFIX		= "\t\t\t\t\t";
	...
	<!-- L2TP  -->
		...
		<tr>
			...
			<td class="l_tb">&nbsp;
				<input type=password id=password name=password size=32 maxlength=255 value="<?=$cfg_l2tp_pass?>">
			</td>
		</tr>
		<tr>
			<td class="r_tb" width=150><?=$m_l2tp_retype_pwd?> :</td>
			<td class="l_tb">&nbsp;
				<input type=password id=password_v name=password_v size=32 maxlength=255 value="<?=$cfg_l2tp_pass?>">
			</td>
		</tr>
		<tr>
			<td class="r_tb" width=150><?=$m_ppp_idle_time?> :</td>
...
<?						$PREFIX		= "\t\t\t\t\t";
	...
	<!-- WWAN 3g  -->
		<div class="box" id="show_wwan" style="display:none">
		<h2><?=$m_title_wwan?></h2>
		<form name="frm_wwan" id="frm_wwan" method="post" action="<?=$MY_NAME?>.php" onsubmit="return check();">
		<input type="hidden" name="ACTION_POST" value="WWAN">
		<p><?=$m_desc_wwan?></p>
		<table cellSpacing=1 cellPadding=1 width=525 border=0>
<!--
		<tr>
			<td class=r_tb><?=$m_driver?> :</td>
			<td class=l_tb>
				<div id="div_modemtype"></div>
			</td>
		</tr>
		<tr>
			<td class=r_tb><?=$m_manufacturer?> :</td>
			<td class=l_tb>
				<div id="div_modemmanu"></div>
			</td>
		</tr>
		<tr>
			<td class=r_tb><?=$m_product?> :</td>
			<td class=l_tb>
				<div id="div_modemprod"></div>
			</td>
		</tr>
-->
		<tr><td>&nbsp;</td></tr>
		<tr>
			<td class="r_tb"><?=$m_country?> :</td>
			<td class="l_tb">&nbsp;
				<input type="hidden" name="country">
				<select name="SearchCountry" id="SearchCountry" onChange="javascript:change_country();">
				<option value=""><?=$m_choice_country?></option>
				</select>
			</td>
		</tr>
		<tr>
			<td class="r_tb"><?=$m_isp?> :</td>
			<td class="l_tb">&nbsp;
				<input type="hidden" name="isp">
				<select name="SearchISP" id="SearchISP" onChange="javascript:change_isp();">
				<option value=""><?=$m_choice_isp?></option>
				</select>
			</td>
		</tr>
<!--
		<tr>
			<td class="r_tb"><?=$m_pin_code?> :</td>
			<td class="l_tb">&nbsp;
				<input type=text id=pin_code name=pin_code size=30 maxlength=255 value="<?=$cfg_wwan_pin_code?>">
				&nbsp;<?=$m_optional?>
			</td>
		</tr>
-->
		<tr>
			<td class="r_tb"><?=$m_username?> :</td>
			<td class="l_tb">&nbsp;
				<input type=text id=username name=username size=30 maxlength=255 value="<?=$cfg_wwan_username?>">
				&nbsp;<?=$m_optional?>
			</td>
		</tr>
		<tr>
			<td class="r_tb"><?=$m_password?> :</td>
			<td class="l_tb">&nbsp;
				<input type=password id=password name=password size=30 maxlength=255 value="<?=$cfg_wwan_password?>">
				&nbsp;<?=$m_optional?>
			</td>
		</tr>
<!-- mark vPassword for D-Link
		<tr>
			<td class="r_tb" width=150><?=$m_retype_pwd?> :</td>
			<td class="l_tb">&nbsp;
				<input type=password id=password_v name=password_v size=30 maxlength=255 value="<?=$cfg_wwan_password?>">
				&nbsp;<?=$m_optional?>
			</td>
		</tr>
-->
		<tr>
			<td class="r_tb"><?=$m_dial_num?> :</td>
			<td class="l_tb">&nbsp;
				<input type=text id=dial_num name=dial_num size=30 maxlength=255 value="<?=$cfg_wwan_dial_num ?>">
			</td>
		</tr>
		<tr>
			<td class="r_tb"><?=$m_auth_proto?> :</td>
			<td class="l_tb">&nbsp;				
				<select name="auth_proto" id="auth_proto">
					<option value="1"<? if ($cfg_wwan_auth_proto==1) {echo " selected";} ?>><?=$m_wwan_auth_auto?></option>
					<option value="2"<? if ($cfg_wwan_auth_proto==2) {echo " selected";} ?>><?=$m_wwan_pap?></option>
					<option value="3"<? if ($cfg_wwan_auth_proto==3) {echo " selected";} ?>><?=$m_wwan_chap?></option>
				</select>
			</td>
		</tr>
		<tr>
			<td class="r_tb"><?=$m_apn?> :</td>
			<td class="l_tb">&nbsp;
				<input type=text id=apn name=apn size=30 maxlength=255 value="<?=$cfg_wwan_apn ?>">
				&nbsp;<?=$m_optional?>
			</td>
		</tr>
		<tr>
			<td class="r_tb"><?=$m_wwan_reconn_mode?> :</td>
			<td class="l_tb">&nbsp;
				<input type=radio name=ppp_conn_mode id=ppp_conn_mode value=0 onclick="chg_wwan_conn_mode(this.form);"><?=$m_always_on?>
				<input type=radio name=ppp_conn_mode id=ppp_conn_mode value=1 onclick="chg_wwan_conn_mode(this.form);"><?=$m_manual?>
				<input type=radio name=ppp_conn_mode id=ppp_conn_mode value=2 onclick="chg_wwan_conn_mode(this.form);"><?=$m_on_demand?>
				<input type=hidden name=reconnmode id=reconnmode>
			</td>
		</tr>
		<tr>
			<td class="r_tb" width=150><?=$m_ppp_idle_time?> :</td>
			<td class="l_tb">&nbsp;
			<input type=text id=idletime name=idletime size=5 maxlength=4 value="<?=$cfg_wwan_idle_time?>">&nbsp;<?=$m_wwan_minutes?>
			</td>
		</tr>
		<tr>
			<td class="r_tb"><?=$m_mtu?> :</td>
			<td class="l_tb">&nbsp;
				<input type=text id=mtu name=mtu maxlength=4 size=5 value="<?=$cfg_wwan_mtu?>">&nbsp;<?=$m_wwan_bytes?>(128-1492)
			</td>
		</tr>
		</tr>
<!--
		<tr>
			<td class="r_tb"><?=$m_auto_reconn?> :</td>
			<td class="l_tb">&nbsp;		
				<input type=radio value=1 id=auto_reconn name=auto_reconn onclick=keep_alive_mode("frm_wwan")><?=$m_enable?>
				<input type=radio value=0 id=auto_reconn name=auto_reconn onclick=keep_alive_mode("frm_wwan")><?=$m_disable?>
			</td>
		</tr>
		<div id="keepalive_wwan" style="display:none">
		<tr>
			<td class="r_tb"><?=$m_keep_alive_int?> :</td>
			<td class="l_tb">&nbsp;
				<input type=text id=keep_alive_int name=keep_alive_int size=6 maxlength=10 
					value="<?=$cfg_wwan_keep_alive_int?>">
				&nbsp;<?=$m_seconds?>
			</td>
		</tr>
		<tr>
			<td class="r_tb"><?=$m_keep_alive_server1?> :</td>
			<td class="l_tb">&nbsp;
				<input type=text id=keep_alive_server1 name=keep_alive_server1 size=30 maxlength=255 
					value="<?=$cfg_wwan_keep_alive_serv1?>">
				&nbsp;<?=$m_optional?>
			</td>
		</tr>
		<tr>
			<td class="r_tb"><?=$m_keep_alive_server2?> :</td>
			<td class="l_tb">&nbsp;
				<input type=text id=keep_alive_server2 name=keep_alive_server2 size=30 maxlength=255 
					value="<?=$cfg_wwan_keep_alive_serv2?>">
				&nbsp;<?=$m_optional?>
			</td>
		</tr>
-->
