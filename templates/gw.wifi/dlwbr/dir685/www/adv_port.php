...
?>
<script>
var port_list = new Array("21","80","443","53","25","110","23","500","1723","801","800","1720","3389"/*,"9"*/);//netmeeting"1720",
var prot_list = new Array("0","0","0","1","0","0","0","1","0","0","0","0","0"/*,"1"*/);
...
function check()
{
	...
	for(i=1; i < rules ; i++)
	{
		if (!is_blank(get_obj("name_"+i).value))
		{
			...
			//IP address shouldn't same with boardcast addr			
			if (is_valid_ip2(tmp_ip,"<?=$cfg_netmask?>")==false )
			{
				alert("<?=$a_invalid_ip?>");
				field_focus(get_obj("ip_"+i), "**");
				return false;
			...
			if (net1[0] != net2[0])
			{
				alert("<?=$a_ip_in_different_subnet?>"+"("+net2[0]+")");
				//alert("IP address for "+get_obj("name_"+i).value+" should be within LAN subnet ("+net2[0]+")");
				field_focus(get_obj("ip_"+i), "**");
				return false;
			}
			
			//IP address shouldn't same with subnet addr
			if(tmp_ip==net2[0])
			{
				alert("<?=$a_invalid_ip?>");
				field_focus(get_obj("ip_"+i), "**");
