...
function check()
{
	...
	for(i=1; i < rules; i++)
	{
		...
		if (!is_blank(tmp_fw_description))
		{
			...
			else if (is_valid_ip2(tmp_src_startip,"<?=$cfg_netmask?>")==false )
			{
				alert("<?=$a_invalid_ip?>");
				field_focus(get_obj("src_startip_"+i), "**");
				return false;
			...
			if(tmp_src_inf == "0")
 			{
 				...
 				alert("<?=$a_not_select_inf?>");
 				field_focus(get_obj("src_inf_"+i), "**");
 				return false;				
 			}
			...
			else if (is_valid_ip2(tmp_dst_startip,"<?=$cfg_netmask?>")==false )
			{
				alert("<?=$a_invalid_ip?>");
				field_focus(get_obj("dst_startip_"+i), "**");
				return false;
			...
			if(tmp_dst_inf == "0")
 			{
 				//alert("Please select Interface");
 				alert("<?=$a_not_select_inf?>");
 				field_focus(get_obj("dst_inf_"+i), "**");
 				return false;				
 			}
			if (tmp_dst_inf=="1")
			{
