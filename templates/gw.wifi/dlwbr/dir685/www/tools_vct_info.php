...
<script>
port_id="<?=$port_id?>";
linkType=["<?=$m_disconnected?>","<?=$m_100full?>","<?=$m_100half?>","<?=$m_10full?>","<?=$m_10half?>","<?=$m_1000full?>","<?=$m_1000half?>"];
...
function getInfo()
{
	...
	case "1":
		...
		break;
	default:
		lan_id=eval(parseInt("<?=$port_id?>", [10])-1);
		if(lan_id==1)		lan_id=4;
		else if(lan_id==2)	lan_id=3;
		else if(lan_id==3)	lan_id=2;
		else if(lan_id==4)	lan_id=1;
		cable_name="<?=$m_lan?>"+lan_id;
