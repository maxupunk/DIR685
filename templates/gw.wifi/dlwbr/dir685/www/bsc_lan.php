	...
	if ($ACTION_POST!="" || $del_id!="")
	{
		...
		else
		{
			...
			if ($dnsr_dirty > 0) //kwest: to prevent restart dhcpd twice which would cause running two dhcpd process sometimes.
			{
				...
				if(query("/gzone/enable")=="1")	{$SUBMIT_STR=$SUBMIT_STR.";submit GZONE";}
			}
		}
		if($lan_ip_dirty > 0)
		{
...
function check()
{
	...
	if ("<?query("/runtime/wan/inf:1/ip");?>" != "" && "<?query("/runtime/wan/inf:1/netmask");?>" != "")
	{
		hzone = get_network_id(f.ipaddr.value, f.netmask.value);
		wanzone = get_network_id("<?query("/runtime/wan/inf:1/ip");?>", "<?query("/runtime/wan/inf:1/netmask");?>");
		if (hzone[0] == wanzone[0])
		{
			alert("<?=$a_lan_wan_conflict?>");
			field_focus(f.ipaddr, "**");
			return false;
		}
	}
