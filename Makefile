...
all:
	...
	echo "==================================================="; \
	echo -e -n "Do you want to rebuild the linux kernel ? (yes/no) : \033[0m"; \
	read answer; \
	if [ "$$answer" == "yes" ]; then	\
...
ROOTFS_DIRS:=	bin dev home lib mnt proc usr var www sys \
...
#	cp configs/wrgns01_dlwbr_dir685.config .config
#	$(Q)make -C $(CONFIG)
