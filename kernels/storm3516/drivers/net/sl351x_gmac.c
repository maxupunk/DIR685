...
#define RTL8366SR
#ifdef RTL8366SR
#include "rtl8366s/rtl8366s_asicdrv.h"
#include "rtl8366s/rtl8366s_api.h"
#include "rtl8366s/rtl8366s_api_ext.h"
...
extern int32 rtl8366s_getPartnerAbility(uint32 phy, phyAbility_t* ability);
...
static int portMasktoPort(uint32 portmask)
{
	int i;
	for(i=0;i<5;i++)
	{
...
static void linkInterruptHandle(void) 
{
	...
	if(rtl8366s_getInterruptStatus(&portmask)!=0)
		return;
	rtl8366sr_printk("switch event %x\n",portmask);
	if(!(portmask & 0x1f))/*port 0~4 LAN port*/
		return;
	port = portMasktoPort(portmask);
	
	if(port >=0 && rtl8366s_getPHYLinkStatus(port,&status)==0)
	{
		//add by sam_pan
		int key=0x30;
		key_handler(key); 
	...
	if(port != 4 )
		return;
	/*is port 4 wan port need sync the gmac and PHY*/
	tp = (GMAC_INFO_T *)&toe_private_data.gmac[GMAC_PORT1];/*wan is gmac 1*/
	if(tp->dev)
		gmac_get_rtl8366sr_status(tp->dev);
	else
		rtl8366sr_printk("%s error\n",__FUNCTION__);
}
static int gmac_open (struct net_device *dev)
{
	GMAC_INFO_T  *tp = (GMAC_INFO_T *)dev->priv;
	...
	if(tp->port_id == GMAC_PORT1)
	{
		...
		if(rtl8366s_setInterruptControl(1/*low triger*/,0x0/*0 is enable for 8366sr*/)==0)
	...
	if(!FLAG_SWITCH)
#else	
	if(tp->auto_nego_cfg)//if auto we need monitor PHY
#endif	
	{
//		init_waitqueue_head (&tp->thr_wait);
//		init_completion(&tp->thr_exited);
...
void gmac_get_rtl8366sr_status(struct net_device *dev)
{
	...
	{
	GMAC_CONFIG0_T	config0,config0_mask;
		phyAbility_t    phyA, linkPartnerAb;
        ...
        rtl8366s_getPHYLinkStatus(4, &linkStatus);
    	    ...
    	    if(rtl8366s_getAsicPortLinkState(4,&speed,&duplex,&link,&txPause,&rxPause,&nWay)!=0)
	{
            ...
            rtl8366s_getPartnerAbility(4, &linkPartnerAb);
    	        ...
    	        if (tp->flow_control_enable == 0)
	{
    	    ...
    	    if (tp->pre_phy_status == LINK_DOWN)
    	    {
				...
				set_bit(__LINK_STATE_START, &dev->state);
				storlink_ctl.link[tp->port_id] = 1;
				rtl8366sr_printk("GMAC-%d link up\n",tp->port_id);
    	    }
    	}
    	else
    	{
    	    ...
    	    if (tp->pre_phy_status == LINK_UP)
    	    {
				...
				clear_bit(__LINK_STATE_START, &dev->state);
				storlink_ctl.link[tp->port_id] = 0;
				rtl8366sr_printk("GMAC-%d link down\n",tp->port_id);
    	    }
    	}
    	tp->full_duplex_status = status.bits.duplex;
		tp->speed_status = status.bits.speed;
