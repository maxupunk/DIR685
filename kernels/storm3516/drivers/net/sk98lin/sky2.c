	...
	if (pAC->RxPort[Port].ReceivePacketTable   != NULL) {
		...
		while (pSkPacket != NULL) {
			if ((pSkPacket->pFrag) != NULL) {
				...
				(dma_addr_t) pSkPacket->pFrag->pPhys-CPU2_BUS_OFFSET,
	...
	if (!IsGoodPkt) {
		...
		*/
		...
		pci_unmap_single(pAC->PciDev,
				 pSkPacket->pFrag->pPhys-CPU2_BUS_OFFSET,
...
#ifdef FOR_CPU2
		pci_unmap_page(pAC->PciDev,
				(dma_addr_t) pFrag->pPhys-CPU2_BUS_OFFSET,
				pFrag->FragLen,
				PCI_DMA_FROMDEVICE);
#else
		pci_unmap_page(pAC->PciDev,
...
SK_AC *pAC)  /* pointer to adapter control context */
{
	...
	*/
	...
	pci_free_consistent(pAC->PciDev, pAC->SizeOfAlignedLETables,
			    pVirtMemAddr, pPhysMemAddr-CPU2_BUS_OFFSET);
#else
	pci_free_consistent(pAC->PciDev, pAC->SizeOfAlignedLETables,
