...
extern void qh_refresh (struct ehci_hcd *ehci, struct ehci_qh *qh);
/*-------------------------------------------------------------------------*/
...
#if !defined  (CONFIG_USB_SL2312_1) && !defined (CONFIG_USB_SL2312_1_MODULE)
	if (usecs > 100)
		printk("uframe %d sched overrun: %d usecs\n",
...
//					frame, type);
                                printk("periodic frame %d bogus type %d\n",frame, type);
			}
...
static int qh_link_periodic (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	...
	ehci->hcd.self.bandwidth_allocated += qh->period
...
printk( "iso request %p too long\n", urb);
		status = -EFBIG;
		goto fail;
	}

	if ((stream->depth + sched->span) > mod) {
printk( "request %p would overflow (%d+%d>%d)\n",
...
)
{
	...
	ehci->hcd.self.bandwidth_isoc_reqs++;
...
sitd_submit (struct ehci_hcd *ehci, struct urb *urb,
	unsigned mem_flags)
{
	printk("split iso support is disabled\n");
	return -ENOSYS;
}
...
restart:
		...
		while (q.ptr != NULL) {
			...
			live = HC_IS_RUNNING (ehci->hcd.state);
			switch (type) {
			case Q_TYPE_QH:
