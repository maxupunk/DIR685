...
//		retval = consistent_alloc(gfp_flags, bytes, dma);//Bruce;;05032005;;
                pci_alloc_consistent(NULL, bytes,dma);
	//} 
	//else
	//	retval = kmalloc(bytes, gfp_flags);
...
//		consistent_free(buf, bytes, dma);//Bruce;;05032005;;
                pci_free_consistent(NULL, bytes,buf, dma);
...
static int CX_dma_Directly(u8 *pu8Buffer, u32 u8Num,u8 bdir)
{
  ...
  //<2>.Map the DMA Buffer
		...
		if (u8Num % 16)
           temp++;
//           if (pu8Buffer <= 0xc0369e24 && (pu8Buffer+temp*16)>= 0xc0369e24 )
//           {
//           	printk("pu8Buffer %x",pu8Buffer);
//            }	
...
// Output:none
//****************************************************
static void Ep_ISR(struct FTC_udc *dev,u8 epNum)
{
  struct FTC_request	*req;
  struct FTC_ep *ep;
 ...
 //<1>.Check request & ep & dev
  ...
  //Check CX 0 bytes
	...
	if (ep->dma && _req->dma == DMA_ADDR_INVALID) {
		...
		if (_req->length % 16)
           temp++;
//           if (_req->buf <= 0xc0369e24 && (_req->buf+temp*16)>=0xc0369e24)
//           {
//           	printk("_req->buf %x",_req->buf);
//            }	
...
//	printk("dev->gadget->ep_list %x\n",dev->gadget.ep_list);
	 ...
	 */
	udc_enable(dev);
        printk("udc_enable dev->gadget %x\n",&dev->gadget);
//For OTG;;Start
	if (dev->transceiver) {
