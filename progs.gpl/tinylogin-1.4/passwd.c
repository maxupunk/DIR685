...
static int my_new_password(const struct passwd *pw, const char *pwd, int algo);

int get_algo(char *a)
{
	int x = 0;					/* standart: md5*/
								//Tim 20071102+
...
extern int update_passwd(const struct passwd *pw, char *crypt_pw)
{
	...
	if (has_shadow) {
		//marco read file system, so we put the tmp file in /var/shadow
	//	snprintf(filename, sizeof filename, "%s", "/var/tmp/shadow");
...
//	out_fp = fopen("/var/shadow+", "w");
	...
	//marco read file system, so we use copy rather than rename
	...
	{
		sprintf(command,"cp %s %s",buf,filename);
		system(command);
		fcntl(fileno(fp), F_SETLK, &lock);
		fclose(fp);
...
extern int passwd_main(int argc, char **argv)
{
	...
	while ((flag = getopt(argc, argv, "a:n:dlu")) != EOF) {
		switch (flag) {
		case 'a':
...
//royc end
	...
	if (!(dflg || lflg || uflg)) {
		...
		if (set_passwd == 1)
		{
			if (my_new_password(pw, newpwd, algo)) {
...
int my_passwd_main(const char *login, const char *pwd)
{
	int amroot;
	char *cp;
	char *np;
...
//	}
	...
	if (!(dflg || lflg || uflg)) {
		...
		printf("Changing password for %s\n", name);
		if (my_new_password(pw, pwd, algo)) {
			error_msg_and_die( "The password for %s is unchanged.\n", name);
		}
	} 
	else if (lflg) {
...
static int my_new_password(const struct passwd *pw, const char *pwd, int algo)
{
	...
	safe_strncpy(pass, pwd, sizeof(pass));
	//Tim 20071102+
	if (algo == 0) 
	{
