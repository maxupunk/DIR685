...
/* shadow in use? */
static int shadow_enabled = 1;

#endif

static int my_adduser(const char *filename, struct passwd *p, char *pwd);
extern int my_passwd_main(const char *login, const char *pwd);
...
//royc add
static void my_passwd_wrapper(const char *login, const char *pwd)
{
	my_passwd_main(login, pwd);
}
//royc end
...
static int adduser(const char *filename, struct passwd *p)
{
	...
	//addgroup_wrapper(p->pw_name, p->pw_gid);  //jack20060507-
...
//	}
	...
	//royc add
	//passwd_wrapper(p->pw_name);
	my_passwd_wrapper(p->pw_name, pwd);
	//royc end
...
//jack20060505 end
	...
	set_passwd = 0;
	//royc end
	while ((opt = getopt (argc, argv, "h:g:s:p:")) != -1) {
		switch (opt) {
			case 'h':
				home = optarg;
				break;
...
//jack20060505 end			
	...
	if(set_passwd == 1)
	{
		return my_adduser(PASSWD_FILE, &pw, (char*)pwd);
	}
	else
	{
...
/* $Id: adduser.c,v 1.2 2007/11/02 04:05:06 jack Exp $ */
