...
#ifdef CONFIG_SL2312_RECVFILE
        {"use receivefile", P_BOOL, P_LOCAL, &sDefault.bUseReceivefile, NULL, NULL, FLAG_ADVANCED | FLAG_SHARE},
#endif
...
static void init_globals(BOOL first_time_only)
{
	...
	//debug_Aaron on 07/17/2007 shrink log size to save Ramdisk space
	//Globals.max_log_size = 5000;
	Globals.max_log_size = 20;
...
{
	char *delim, *quote;
	if(Globals.szPassdbBackend==NULL)
	goto out;
...
#ifdef CONFIG_SL2312_RECVFILE
FN_LOCAL_BOOL(_lp_use_receivefile, bUseReceivefile)
#endif
...
#ifdef CONFIG_SL2312_RECVFILE
BOOL lp_use_receivefile(int snum)
{
        extern enum protocol_types Protocol;
        /* Using receivefile blows the brains out of any DOS or Win9x TCP stack... JRA. */
        if (Protocol < PROTOCOL_NT1) {
          return False;
        }
        return (_lp_use_receivefile(snum) && (get_remote_arch() != RA_WIN95) && !srv_is_signing_active());
}

void set_use_receivefile(int snum, BOOL val)
{
//debug_Aaron
//DEBUG(0, ("%s:snum=%d, val=%d", __func__, snum, val) );
        if (LP_SNUM_OK(snum))
                ServicePtrs[snum]->bUseReceivefile = val;
        else
           sDefault.bUseReceivefile = val;
}
#endif
