			...
			SMB_OFF_T offset, size_t n)
{
 ...
 //debug_Aaron on 05/30/2006 pass 1 to indicate it's call from sendfile not receivefile
	result = sys_sendfile(tofd, fromfd, hdr, offset, n, 1);
...
//debug_Aaron on 04/07/2005, fix the bug of FAT32 lseek() to slow problem
 //             Sympton: copy files bigger than 400MB from PC to server will cause oplock_break failed
#if 0
...
static int vfswrap_ftruncate(vfs_handle_struct *handle, files_struct *fsp, int fd, SMB_OFF_T len)
{
 ...
 currpos = SMB_VFS_LSEEK(fsp, fd, 0, SEEK_CUR);
        ...
        DEBUG(10, ("vfswrap_ftruncate(), after SMB_VFS_LSEEK(), currpos=%ld\n", currpos));
        if (currpos == -1)
        {
             result = 0;
             goto done;
        ...
        /* Do an fstat to see if the file is longer than the requested
            size in which case sys_ftruncate() should be called */
         if (SMB_VFS_FSTAT(fsp, fd, &st) == -1)
         {
               DEBUG(10, ("vfswrap_ftruncate(), after SMB_VFS_FSTAT(), result=-1\n"));
               result = 0;
                 goto done;
         }

         DEBUG(10, ("vfswrap_ftruncate(), size=%ld\n", st.st_size));

#ifdef S_ISFIFO
        if (S_ISFIFO(st.st_mode))
        {
                DEBUG(1, ("vfswrap_ftruncate(), is FIFO mode\n"));
                result=0;
                goto done;
        }
#endif

       if (st.st_size <= len) /* the file size is less than requested size already */
       {      
               result = 0;
                 goto done;
         }
