...
#endif
//+++joel for smbpasswd link with smbd together.
#if 1
    if(strcmp(argv[1],"smbpasswd")==0)
        return main_with_smbd(argc-1,&argv[1]);
    else if(strcmp(argv[0],"smbpasswd")==0)
        return main_with_smbd(argc,&argv[0]);
    if(strcmp(argv[1],"nmbd")==0)
        return nmbd_main(argc-1,&argv[1]);
    else if(strcmp(argv[0],"nmbd")==0)
        return nmbd_main(argc,&argv[0]);
#if CONFIG_USER_SAMBA_CLIENT
	else  if(strcmp(argv[0],"smbclient")==0)
        return smbclient_main(argc,&argv[0]);
	else  if(strcmp(argv[0],"smbtree")==0)
        return smbtree_main(argc,&argv[0]);
    else  if(strcmp(argv[0],"nmblookup")==0)
        return nmblookup_main(argc,&argv[0]);
