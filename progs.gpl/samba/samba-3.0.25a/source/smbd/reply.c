		...
		ssize_t mincount, char *outbuf, int out_buffsize)
{
	 ...
	 */
//debug_Aaron on 12/27/2004, fix the bug of Netbench failed when sendfile=yes and TSS is ON, get from samba-3.0.5
	//if ( (chain_size == 0) && (nread > 0) &&
	//    (fsp->wcp == NULL) && lp_use_sendfile(SNUM(conn)) ) {
 	if ((nread > 0) && (fsp->wcp == NULL) &&
		...
		files_struct *fsp, SMB_OFF_T startpos, size_t smb_maxcnt)
{
 ...
 //debug_Aaron on 12/27/2004, fix the bug of Netbench failed when sendfile=yes and TSS is ON, get from samba-3.0.5
	...
	if ((CVAL(inbuf,smb_vwv0) == 0xFF) && (EXCLUSIVE_OPLOCK_TYPE(fsp->oplock_type) ||
        	     LEVEL_II_OPLOCK_TYPE(fsp->oplock_type)) &&
                       lp_use_sendfile(SNUM(conn)) && (fsp->wcp == NULL) ) {
...
int reply_write_and_X(connection_struct *conn, char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	if (IS_IPC(conn)) {
                ...
                int ret;
                if (numtowrite >= 1024) {
                        ret = read_data(smbd_server_fd(), inbuf+4+smb_doff, smblen-smb_doff);
                        if (ret != (smblen-smb_doff)) {
                                DEBUG(0, ("reply_write_and_X, read data error, towrite %d, ret %d\n", numtowrite-smb_doff, ret));
...
#ifdef CONFIG_SL2312_RECVFILE
        {
                //DEBUG(0,("rcv. pos %ld, cnt %ld\n", startpos, numtowrite));
                if (lp_use_receivefile(SNUM(conn)) && (numtowrite >= 1024)
#if !(defined(HAVE_EXPLICIT_LARGEFILE_SUPPORT) && defined(HAVE_OFF64_T) && defined(HAVE_SENDFILE64))
                        && ((startpos + numtowrite) <= ((SMB_OFF_T)0x7FFFFFFF))
#endif
                       )
                {
                          //debug_Aaron on 05/30/2006 pass 0 to sys_sendfile() indicates this is receivefile
                        nwritten = sys_sendfile(fsp->fh->fd, smbd_server_fd(), 0, startpos, numtowrite, 0);

                        if (nwritten == -1) {
                                DEBUG(0, ("Recvfile failed. Server fd %d, fnum %d, numtowrite %d\n",
                                      smbd_server_fd(), fsp->fnum, (int)numtowrite));
                                return -1;
                        }
                } else {
                        if (numtowrite >= 1024) {
                                read_data(smbd_server_fd(), inbuf+4+smb_doff, smblen-smb_doff);
                        }
