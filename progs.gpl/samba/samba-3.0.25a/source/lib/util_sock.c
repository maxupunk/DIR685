...
BOOL receive_smb_raw(int fd, char *buffer, unsigned int timeout)
{
	...
	if(len > 0) {
                ...
                //debug_Aaron
                //DEBUG(0,("receive_smb_raw: smb_size=%d, smb_vwv11=%d, len=%d\n", smb_size, smb_vwv11, len));
                #define READ_AHEAD_LEN  59
                ...
                if (len > smb_size)
                {
                        ...
                        else
                        {
                                if (timeout > 0)
                                {
                                        ret = read_socket_with_timeout(fd, buffer+4, READ_AHEAD_LEN, READ_AHEAD_LEN, timeout);
                                }       
                                else
                                {
                                        ret = read_data(fd, buffer+4, READ_AHEAD_LEN);
                                }       
                                
                                if (ret != READ_AHEAD_LEN)
                                        goto error;
                                        
                                unsigned int cmd_type = CVAL(buffer, smb_com);
                                unsigned int smb_doff = CVAL(buffer, smb_vwv11);
                                
                                if ((cmd_type & 0xff)!= 0x2f)
                                {
                                        ret = read_data(fd, buffer+4+READ_AHEAD_LEN, len-READ_AHEAD_LEN);
                                        if (ret != (len-READ_AHEAD_LEN))
                                                goto error;
                                }               
                                else
                                {
                                        if (smb_doff > READ_AHEAD_LEN)
                                        {
      			                     //DEBUG(0,("receive_smb_raw: smb_doff=%d, cmd_type=%d\n", smb_doff, cmd_type));
                                                 ret = read_data(fd, buffer+4+READ_AHEAD_LEN, smb_doff - READ_AHEAD_LEN);
                                                if (ret != (smb_doff - READ_AHEAD_LEN))
                                                        goto error;
                                        }
                                }
                        }
