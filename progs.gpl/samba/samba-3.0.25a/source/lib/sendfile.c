...
//debug_Aaron on 05/30/2006 add istx for distinguish call from sendfile or receivefile 
//ssize_t sys_sendfile(int tofd, int fromfd, const DATA_BLOB *header, SMB_OFF_T offset, size_t count)
ssize_t sys_sendfile(int tofd, int fromfd, const DATA_BLOB *header, SMB_OFF_T offset, size_t count, int istx)
{
	size_t total=0;
	ssize_t ret;
	size_t hdr_len = 0;

	//debug_Aaron on 05/30/2006 timeout for receivefile
	struct timeval to;	
	time_t starttime = time(NULL);
	int timeout = (OPLOCK_BREAK_TIMEOUT+OPLOCK_BREAK_TIMEOUT_FUDGEFACTOR) * 60;
	...
	if (header) {
 	...
 	//debug_Aaron on 05/30/2006 for debugging
        //DEBUG(0,("sys_sendfile: pid=%d, before sendfile64(), count=%d, istx=%d\n", sys_getpid(), count, istx));
	...
	while (total) {
		...
		do {
			...
			if (nwritten == -1 && errno == EINTR)
           			DEBUG(0,("sys_sendfile: pid=%d, after sendfile64() return %d\n", sys_getpid(), nwritten));
		...
		if (nwritten == -1) {
			...
			//debug_Aaron on 05/30/2006 for debugging
                        DEBUG(0,("sys_sendfile: pid=%d, sendfile64() return -1, errno=%d\n", sys_getpid(), errno));
			return -1;
		}
		if (nwritten == 0)
		{
			if (istx == 0)
			{
				...
				{
					//debug_Aaron on 05/30/2006 for debugging
                       			//DEBUG(0,("sys_sendfile: pid=%d, sendfile64() return 0, timeout = %d, try again!!\n", 
					//	sys_getpid(), timeout));

					//debug_Aaron on 07/24/2006 wait for a while to fix the bug caused by TX/RX the NAS 
					//		at the sam time, do not wait too long otherwise will cause Netbench
					//		performance drop
					smb_msleep(1);
          	                     	continue ;
				}
			 	...
			 	//debug_Aaron on 05/30/2006 for debugging
                        	DEBUG(0,("sys_sendfile: pid=%d, sendfile64() return 0, timeout giveup !!\n", sys_getpid()));
			...
			{
				//debug_Aaron on 05/30/2006 for debugging
                                DEBUG(0,("sys_sendfile: pid=%d, sendfile64() return 0 for TX !!\n", sys_getpid()));
	...
	//debug_Aaron on 05/30/2006 for debugging
        //DEBUG(0,("sys_sendfile: pid=%d, after sendfile64(), count=%d, istx=%d\n", sys_getpid(), count, istx));
