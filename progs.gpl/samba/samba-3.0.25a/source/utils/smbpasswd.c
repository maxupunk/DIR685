...
/*********************************************************
 Start here.
**********************************************************/
//+++joel add for read file to add users ,can speed up.
#if 1
int Process_from_file(FILE *fp)
{
	int local_flags = 0,argc=5;
    ...
    while(fgets(buf,256,fp)!=NULL)
    {
		...
		new_passwd = smb_xstrdup(passwd);

		fstrcpy(user_name, username);
        ...
        if (!init_names())
        {
		    fprintf(stderr, "init_names error \n");
		    return 1;
		}

	    /* Check the effective uid - make sure we are not setuid */
	    if (is_setuid_root()) {
		fprintf(stderr, "is_setuid_root\n");
		return 0;
	    }
...
{	
	...
	load_case_tables();//marco, it must initial first
#if defined(HAVE_SET_AUTH_PARAMETERS)
	set_auth_parameters(argc, argv);
#endif /* HAVE_SET_AUTH_PARAMETERS */
    //fprintf(stderr,"main_with_smbd\n");
	if (getuid() == 0) {
		local_flags = LOCAL_AM_ROOT;
	}
//+++joel add    
    ...
    {
        fprintf(stderr,"add passwd from file\n");    
        if((fp=fopen(argv[2],"r"))==NULL)
        return 1;
        local_flags|= LOCAL_ADD_USER;
          Process_from_file(fp);
        fclose(fp);
       return 0;
    }
#endif    
