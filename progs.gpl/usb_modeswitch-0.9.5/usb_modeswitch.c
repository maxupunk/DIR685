...
static struct option long_options[] =
{
	...
	{"Storagename",			required_argument, 0, 's'},
	{0, 0, 0, 0}
};
...
void eject(char *deviceName)
{
	...
	** it must be given the O_NONBLOCK flag when opening. Bison tests showed
	** that if this isn't done, this program will not work.
	*/
	
	if ((cdrom = open(deviceName,O_RDONLY | O_NONBLOCK)) < 0) {
	    perror("open");
	    exit(1);
	}
...
int readArguments(int argc, char **argv)
{
	...
	while (1)
	{
		//+++Mark 2009/1/5 09:25am
//		c = getopt_long (argc, argv, "hWQv:p:V:P:C:m:M:r:d:H:n:c:R:i:u:a:",
		c = getopt_long (argc, argv, "hWQv:p:V:P:C:m:M:r:d:H:n:c:R:i:u:a:e:s:",
		//---Mark
		...
		switch (c)
		{
			...
			case 's': strcpy(g_storage_name,optarg); break;	// usb storage name
			case 'e': ejectCDROM=1; eject(optarg); // the eject function is written by bison
...
//				printf (" -n, --NeedResponse [1|0]      whether to try to read a response - OBSOLETE\n");
				...
				//+++Mark 2009/1/5 09:23am
				printf (" -e, --ejectCDROM [devicename]	eject the CDROM device. Don't use other options when using this.\n\n");
				//---Mark
				//+++Hendry 2009/4/6 
				printf (" -s, --storagename [devicename] for storage interface \n\n");
				//---Hendry
...
int mount_dev(char *source, char *target )
{
	int ret = -1;
	
	if((ret=mount(source, target,"ufsd",0,"umask=000"))==0)
	{	
		printf("usb_switchmode : Mount USB in NTFS fs\n");
	}
	else if((ret=mount(source, target,"ext3",0,""))==0)
	{
		printf("usb_switchmode : Mount USB in ext3 fs\n");
	}
	else if((ret=mount(source, target,"ext2",0,""))==0 )
	{
		printf("usb_switchmode : Mount USB in ext2 fs\n");
	}
	/* hendry modify 5/21/2009 */
	/* support for kernel 2.4, vfat mount works more flexible with no flag */
	else if((ret=mount(source, target,"vfat",0,""))==0 )
	{
		printf("usb_switchmode : Mount USB in vfat fs\n");
	}
	return ret;
}

int is_specific_3G(int vendorID, int productID)
{
	//DWM-156 
	if(vendorID == 0x07d1 && productID == 0xa800) 
	{
		/* 
		1. mount the storage device 
		2. create an empty file "wcdma.cfg" into it 
		3. unmount it so directly effective (prevent being cached) 
		*/
		char command[300];
		char targetmount[30];
		char source[30];
		
		printf("Switching for DWM-156 ..... \n");
		if(g_storage_name[0] == 0)		
		{
			printf("Need to specify storage device name for DWM 156\n");	
			printf("Exit ..\n");	
			return 1;
		}
		sprintf(targetmount, "/var/temp_mount_%d", time(0));
		if(mkdir(targetmount,777)!=0)
		{
			printf("Can't make dir in modeswitch\n");
			return 1;
		}	
		
		sprintf(source, "/dev/%s", g_storage_name);
		//printf("strlen(g_storage_name) : %d, g_storage_name : %s\n", strlen(g_storage_name), g_storage_name);
		//printf("usb_modeswitch : source %s \n", source);
		//printf("usb_modeswitch : target %s \n", targetmount);
		
		if( mount_dev(source, targetmount) == 0)	//if mount success
		{
			sprintf(command, "echo 0 > %s/wcdma.cfg\n", targetmount);
			system(command);
			//printf("command : %s\n", command );
			printf("Switching for DWM-156 success\n");
			
			if(umount(targetmount)<0)
			{
				printf("WARNING umount : %s failed\n", targetmount);
				//TODO 
				return 1;
			}
		}
		else {
			printf("WARNING : vfat fs support maybe disabled. Please open it.\n");
			printf("Switching for DWM-156 failed\n");
		}
...
/* --- hendry */
	...
	if (dev != NULL) {
		...
		/* +++ hendry */
		#if 1 
			/* we force retry if resource/device isn't available yet.*/
			if(retry++ < 3)
			{
				sleep(3);goto find_usb;
			}
		#endif
		/* --- hendry */
	...
	/* hendry 4/10/2009 +++ */
	/* for specific 3G card that have storage interface, such DWM-156. We need to do specific action. */
	if(is_specific_3G(DefaultVendor, DefaultProduct))
		exit(0);
