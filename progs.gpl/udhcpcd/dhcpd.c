...
/* prototype */
int valid_ip(u_int32_t ipaddr,char *chaddr);
...
int valid_ip(u_int32_t ipaddr,char *chaddr)
{
	...
	{
		if(ipaddr==static_leases[j].yiaddr)
		{
			if(memcmp(static_leases[j].chaddr, chaddr, sizeof(static_leases[j].chaddr))==0)
				return 2; // match static
			else
			{
				fprintf(stderr,"Error belong static \n");
				return 0;
			}
		}
	}

	// 4.check dynamic table
	if ((lease = find_lease_by_yiaddr(ipaddr)) && lease_expired(lease))
	{
		DEBUG(LOG_INFO, "%s: Woops !!!",__FUNCTION__);
		return 0;
	}
...
/* 32 bit change to 64 bit dennis 20080311 end */
	...
	{	/* loop until universe collapses */
		...
		case DHCPREQUEST:
			...
			/* +++ Joy added static lease */
			{
				if ((static_lease = find_static_lease_by_chaddr(packet.chaddr)))
				{
					DEBUG(LOG_INFO, "%s: found static lease %02X:%02X:%02X:%02X:%02X:%02X",__FUNCTION__,
					...
					{
						if (requested && requested_align == static_lease->yiaddr)
						{
							/* Set the lease time to infinity */
							add_simple_option(packet.options, DHCP_LEASE_TIME, 0xFFFFFFFF);
							sendACK(&packet, static_lease->yiaddr);
						}
						else
						{
							sendNAK(&packet);
						}
						lease = NULL; //static_lease found, invalidate lease
					}
				}
			}
			/* --- Joy added static lease */
			if (lease)	/*ADDME: or static lease */
			{
				...
				else
				{
					...
					else
					{
						...
						if (lease->yiaddr == packet.ciaddr && valid_ip(packet.ciaddr, (char *)packet.chaddr))
							sendACK(&packet, lease->yiaddr);
						else
			...
			{
				DEBUG(LOG_INFO, "%s: have requested !",__FUNCTION__);
				if (find_static_lease_by_yiaddr(requested_align))
					sendNAK(&packet);
				else
				/* INIT-REBOOT State */
				...
				else if(requested_align && (ret=valid_ip(requested_align, (char *)packet.chaddr)))
				{
					if(ret==2)	//-----static dhcp
						add_simple_option(packet.options, DHCP_LEASE_TIME, 0xFFFFFFFF);
			...
			else if(packet.ciaddr && (ret=valid_ip(packet.ciaddr, (char *)packet.chaddr)))
			{
				if(ret==2)	//-----static dhcp
					add_simple_option(packet.options, DHCP_LEASE_TIME, 0xFFFFFFFF);
...
//+++joel 2004/7/2 04:04¤U¤È when client release should re-write the lease file
				write_leases();
//---joel 2004/7/2 04:04¤U¤È when client release should re-write the lease file
#ifndef LOGNUM
