...
//+++ Teresa update 
	var url = "/api/torrent-add-url?url=" + encodeURIComponent(url);
...
/* Functions for generating HTML fragments */
function attr(name, value) { return " " + name + "=" + qs(value); }
function attcls(value) { return attr("class", value); }
function startTag(tag) { return "<" + tag + ">"; }
function endTag(tag) { return "</" + tag + ">"; }
function inTag(tag, txt, cls) { 
	var t = startTag(tag);
	if (cls)
		t = "<" + tag + attcls(cls) + ">";
	return t + txt + endTag(tag); 
}
function table(txt) { return inTag("table", txt); }
function center(txt) { return inTag("center", txt); }
function qs(txt) { return "\"" + txt + "\""; }
function qs2(txt) { return "'" + txt + "'"; }
function td(txt, cls) { return inTag("td", txt, cls); }
function p(txt) { return inTag("p", txt); }
function tbody(txt) { return inTag("tbody", txt); }
function thead(txt) { return inTag("thead", txt); }
function tr(txt) { return inTag("tr", txt); }
function pre(txt) { return inTag("pre", txt); }
function h3(txt) { return inTag("h3", txt); }
function li(txt, cls) { return inTag("li", txt, cls); }
function ul(txt, cls) { return inTag("ul", txt, cls); }
function span(txt, cls) { return inTag("span", txt, cls); }
function ajs(jsurl, txt) { return "<a href=javascript:" + jsurl + ">" + txt + "</a>"; }
function th(txt, scope, cls) {
	var clsTxt = null;
	var scopeTxt = null;
	if (scope) scopeTxt = attr("scope", scope);
	if (cls) clsTxt = attcls(cls);
	if (clsTxt || scopeTxt) {
		if (null == clsTxt) clsTxt = "";
		if (null == scopeTxt) scopeTxt = "";
		return "<th" + scopeTxt + clsTxt + ">" + txt + "</th>";
	}
	else
		return inTag("th", txt);
}
