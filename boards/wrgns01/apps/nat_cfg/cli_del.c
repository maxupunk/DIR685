...
extern int nat_del_help_cmd(int argc, char ** argv);
static int nat_del_ip_cmd(int argc, char **argv);
static int nat_del_port_cmd(int argc, char **argv);
static int nat_del_rule_cmd(int argc, char **argv);
static int nat_del_eth0_cmd(int argc, char **argv);
static int nat_del_eth1_cmd(int argc, char **argv);

CLICMD_T nat_del_cmds[] = {
    {"?",    	   nat_del_help_cmd,  	"Help commands"},
    {"help", 	   nat_del_help_cmd,  	"Help commands"},
    {"ip",     	   nat_del_ip_cmd,  	"Delete a IP configuration"},
    {"port",       nat_del_port_cmd,	"Delete a TCP/UDP port entry from NAT SW PORT lists"},
    {"rule",       nat_del_rule_cmd,	"Delete a weighting rule from a specified TxQ"},
    {"eth0",       nat_del_eth0_cmd,	"Remove all eth0 IP configuration"},
    {"eth1",       nat_del_eth1_cmd,	"Remove all eth1 IP configuration"},
    {0,0,0}
};

/*----------------------------------------------------------------------
* ROUTINE NAME - nat_del_rule_cmd
*-----------------------------------------------------------------------
* DESCRIPTION: 
...
static int nat_del_rule_cmd( int argc, char ** argv)
{
	...
	if (argc < 11)
	{
    	fprintf(stderr,"Syntax:  nat_cfg del rule qid(1-4) protocol(tcp/udp) sips sipe dips dipe sports(1-65535) sporte dports dporte \n");
    	fprintf(stderr,"Example: nat_cfg del rule 1 tcp 192.168.2.1 192.168.2.100 200.2.2.1 200.2.2.100 1 65535 1 65535 \n");
    	return 1;
	}	  
//qid
    qid = atol(argv[1]) ;
...
static int nat_del_ip_cmd( int argc, char ** argv)
{
	...
	if (argc < 4)
	{
    	fprintf(stderr,"Syntax:  nat_cfg del ip device_Name(eth0 eth1) IP Subnet_Mask(255.255.255.0)\n");
    	fprintf(stderr,"Example: nat_cfg del ip eth1 192.168.1.200 255.255.255.0\n");
    	return 1;
	}	  
...
static int nat_del_port_cmd( int argc, char ** argv)
{
	...
	if (argc < 6)
	{
    	fprintf(stderr,"Syntax:  nat_cfg del port protocol(tcp/udp/both) sports(1-65535) sporte dports dporte \n");
    	fprintf(stderr,"Example: nat_cfg del port tcp 1 65535 1 65535 \n");
    	return 1;
	}
...
static int nat_del_eth0_cmd( int argc, char ** argv)
{
	...
	if (argc < 1)
	{
    	fprintf(stderr,"Example: nat_cfg del ip eth0\n");
    	return 1;
	}

	sprintf(ifr.ifr_name, "eth0");
	ip_data.cmd = NATRMIPCFG;
	ip_data.len = 0;
	ifr.ifr_data = (void *)&ip_data;
...
static int nat_del_eth1_cmd( int argc, char ** argv)
{
	...
	if (argc < 1)
	{
    	fprintf(stderr,"Example: nat_cfg del ip eth1\n");
    	return 1;
	}
