...
/*hendry*/
#define FORMAT_PROGRESS_FILE "/var/format_pgrs"
...
static void write_inode_tables(ext2_filsys fs, int lazy_flag)
{
	...
	/*count for how many times to write to file*/
	//the process will increase 1% for each 'aunit' nodes written
	aunit = (fs->group_desc_count)/100;	
	for (i = 0; i < fs->group_desc_count; i++) {
		/*hendry*/
		if( (i%aunit) == 0)	//write to file if process increase 1 %
		{
			progress_percent = (i*100) /(fs->group_desc_count) ;
			sprintf(tmp, "%d", progress_percent);
			/*write progress to a file*/
			export_to_file(tmp,FORMAT_PROGRESS_FILE);
		}
