...
void Channel_App(IDirectFBEventBuffer *p_eventbuffer, FcNode *guid_node)
{					
	DownloadObj *dobj;
    GridObj *grid;		
	IconObj *icon;	
	...
	//get channel node tree
    serv_response= client_cmd_handler(p_eventbuffer, CHANNEL_GET, guid_node); 
    ...
    //get channel node count
    node_count=get_fcnode_count(fcitem_node);
	...
	icon=create_icon_tree(grid, main_layer, win_dsc);
 	...
 	fc_download_init_v2(fcitem_node);
 						
    //draw first photo, set window opacity and position of window stack 
	draw_image_scale(fcitem_node->local_file_path, screen_space, icon->window_surface, gp_dfb);			
	...
	//create DownloadObj for downloading thread(thread id is tid[0])
	dobj=(DownloadObj*)malloc(sizeof(DownloadObj));
	dobj->node=fcitem_node;
		...
		key = GetInputDeviceEventByType(p_eventbuffer,PTYPE_NOSCREEN_API);    			           									
 		...
 		if( key == DIKI_UP ){
			...
			pthread_cancel_msg(thread_flag);
 			...
 			client_cmd_handler(p_eventbuffer, CHANNEL_THREAD_STOP, NULL);      	
 			free(view);	
 			free(dobj);
 			grid_free(grid);
 			icons_free(icon); 			 			 					
 			remove_fcfile(fcitem_node);
 			fcitem_node=delete_fcnode_tree(fcitem_node);     						     			  			     			
            ...
            //move left
			shift_fcnode_left(fcitem_node);
			fcitem_node=get_fcnode_head(fcitem_node);
                                 
            if(serv_response==WAN_ERROR||serv_response==DNS_ERROR)
			{	
				while(!check_file_exist(fcitem_node->local_file_path)) 
				{
					//move left
					shift_fcnode_left(fcitem_node);
					fcitem_node=get_fcnode_head(fcitem_node);
			...
			if(((fd=open(fcitem_node->local_file_path, O_RDWR))!=-1)&&check_image_available(fcitem_node->local_file_path, gp_dfb))
			{			    			  																					
				if(lockf(fd, F_TEST, 0)==-1)
				{
					dprintf("\n file %s is locked\n", fcitem_node->local_file_path);					
				}else{
				    dprintf("\nleft filename-> %s\n", fcitem_node->local_file_path);
            ...
            //move right		
			shift_fcnode_right(fcitem_node);
			fcitem_node=get_fcnode_head(fcitem_node);	
			...
			dprintf("\nslide show start\n");													
			serv_response=client_cmd_handler(p_eventbuffer, GET_CHANNELS_UPDATE_VERSION, NULL); 
			...
			{	 						
				...
				case WAN_ERROR:
					...
					{	
						...
						//draw photo										
						if(check_file_exist(fcitem_node->local_file_path)&&check_image_available(fcitem_node->local_file_path, gp_dfb))
						{																		
							icon->window_surface->Clear(icon->window_surface, 0, 0, 0, 0);						
							draw_image_scale(fcitem_node->local_file_path, screen_space, icon->window_surface, gp_dfb);									
							icon->window_surface->Flip(icon->window_surface, NULL, DSFLIP_NONE );								
				...
				case DO_UPDATE:
                   //channel-show update mode	
					...
					serv_response= client_cmd_handler(p_eventbuffer, CHANNEL_UPDATE, guid_node); 
					//if(serv_response!=REQ_SUCCESS) goto key_up;					
			...
			//run slide-show function
			p_eventbuffer->Reset(p_eventbuffer);								
			view->node=get_fcnode_head(fcitem_node);				
			view->flag=1;								
			pthread_create(&tid[1], NULL, &view_player_r, view);														
