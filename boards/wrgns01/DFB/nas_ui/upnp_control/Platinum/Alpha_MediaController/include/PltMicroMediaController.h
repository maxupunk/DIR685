...
//joel add for know info    
	...
	void OnGetPositionInfoResult(
        ...
        void*                    /* userdata */);           
//sam_pan add for socket communicate        
	PLT_StringMap GetMediaServerTable(); 	 
	PLT_StringMap GetMediaRendererTable();
	PLT_StringMap ViewMediaContainers();
	void GetMediaServerDevice(const char *chosenUUID);
	void GetMediaRendererDevice(const char *chosenUUID);
	int ChangeMediaDir(const char *newObjID);
	int OpenMediaItem(const char *objID);
	void PlayMediaItem();
	void StopMediaItem();
	void PauseMediaItem();
	void MuteMediaItem();
	void VoiceUpMediaItem();
	void VoiceDownMediaItem();	
	void CdUpMediaItem();	
private:
	...
	void    HandleCmd_mute();
	void    HandleCmd_volume(int up);
	void    HandleCmd_next();
	void    HandleCmd_prev();
	void    HandleCmd_MediaInfo();
	void    HandleCmd_PositionInfo();
private:
